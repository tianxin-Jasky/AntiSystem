		...
		socket.onmessage = function(event)
		{
			...
			for(var i=1,j=0;i<str.length;i++,j++){
				...
				switch (ramainder) {
					...
					case 3:
						mail_judge = str[i];
						console.log(mail_judge);
						console.log("第",mail_Count+1,"次");
						//TODO:用上面已经有的三个元素去给他赋值
						mail_Count++;

						var subject =mail_subject;
						var time=mail_time;
						var sender=mail_sender;
						var judge=mail_judge;
			...
			//console.log("存储之后，查看数据");
			console.log(that.getStorage(2).Mailsubject);
			// for(var i=0;i<mail_number;i++){
			// 	console.log(that.getStorage(i).Mailsubject);
			// }
